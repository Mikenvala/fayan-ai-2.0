...
CASES_JSON          = os.path.join(os.path.dirname(__file__), "..", "extracted_cases", "deduped", "civil_cases.json")
CRIMINAL_CASES_JSON = os.path.join(os.path.dirname(__file__), "..", "extracted_cases", "deduped", "criminal_cases_new.json")
...
class LegalRetriever:
    """
    双库检索器：同时加载民事库 + 刑事库，
    analyze 时由外部根据 CaseClassifier 结果选择对应 retriever。
    """
    def __init__(self, civil_json: str, criminal_json: str):
        self.civil_json  = civil_json
        self.criminal_json = criminal_json
        self._build_index("civil",  civil_json)
        self._build_index("criminal", criminal_json)

    # ------------------------------------------------------------------
    def _build_index(self, kind: str, cases_json: str):
        ...
        def build_search_text(case):
            ...
            parts = [
                case.get("title", "") * 3,
                " ".join(mp.get("keywords", [])) * 2,
                mp.get("ruling_points", "") * 2,
                mp.get("related_laws", "") * 1,
                case.get("content", "") * 1,
            ]
        ...
        search_texts = [build_search_text(c) for c in cases]
        case_ids     = [c.get("case_number", c.get("id", "")) for c in cases]
        tokenized    = [[t for t in jieba.cut(text) if len(t) > 1] for text in search_texts]
        token_strs   = [" ".join(tokens) for tokens in tokenized]

        vectorizer   = TfidfVectorizer(max_features=2048, token_pattern=r'(?u)\b\w+\b')
        tfidf_matrix = vectorizer.fit_transform(token_strs)
        bm25         = BM25Okapi(tokenized)
        bm25.param_b = 0.75
        bm25.param_k1 = 1.5

        setattr(self, f"cases_{kind}",       cases)
        setattr(self, f"search_texts_{kind}", search_texts)
        setattr(self, f"case_ids_{kind}",    case_ids)
        setattr(self, f"tokenized_{kind}",   tokenized)
        setattr(self, f"vectorizer_{kind}",  vectorizer)
        setattr(self, f"tfidf_matrix_{kind}", tfidf_matrix)
        setattr(self, f"bm25_{kind}",         bm25)

    # ------------------------------------------------------------------
    def retrieve(self, query: str, k: int = 5, kind: str = "civil") -> list[dict]:
        """
        从指定库检索 k 条案例。
        kind: "civil" | "criminal"
        """
        tokenized    = getattr(self, f"tokenized_{kind}")
        vectorizer   = getattr(self, f"vectorizer_{kind}")
        tfidf_matrix = getattr(self, f"tfidf_matrix_{kind}")
        bm25         = getattr(self, f"bm25_{kind}")
        cases        = getattr(self, f"cases_{kind}")

        query_tokens  = [t for t in jieba.cut(query) if len(t) > 1]
        query_str     = " ".join(query_tokens)

        bm25_scores_raw = np.array(bm25.get_scores(query_tokens), dtype=float)
        query_vec      = vectorizer.transform([query_str])
        from sklearn.metrics.pairwise import cosine_similarity
        tfidf_scores   = cosine_similarity(query_vec, tfidf_matrix).flatten()

        legal_terms  = [t for t in query_tokens if len(t) >= 3]
        term_bonus   = np.zeros(len(cases))
        if legal_terms:
            for idx, tokens in enumerate(tokenized):
                term_bonus[idx] = sum(1 for t in legal_terms if t in tokens) / max(len(legal_terms), 1)
        ...
        mmr_ranked = []
        cand_dense = tfidf_matrix[top_candidates].toarray().astype(float)
        ...
        for global_idx, mmr_score in mmr_ranked:
            ...
            results.append({
                ...
                "source":      f"mmr_fused_{kind}",
                "case_kind":   kind,
            })
        return results

    @property
    def civil_count(self) -> int:
        return len(getattr(self, "cases_civil", []))

    @property
    def criminal_count(self) -> int:
        return len(getattr(self, "cases_criminal", []))
...
裁判要旨: {case.get('metadata', {}).get('ruling_points', 'N/A')[:300]}
关联法规: {case.get('metadata', {}).get('related_laws', 'N/A')[:200]}
"""
            )
        cases_block = "\n".join(cases_text)
...
class FaYanLegal:
    def __init__(self, api_key: str, base_url: str, model: str,
                 cases_json: str, criminal_json: str):
        self.rule_engine = RuleEngine()
        self.llm = LegalLLM(api_key, base_url, model)
        self.retriever = LegalRetriever(cases_json, criminal_json)
    ...
    def analyze(self, case_text: str,
        ...
        # Step 0: 自动案件分类 → 确定检索库
        classified = CaseClassifier.classify(case_text)
        case_kind = "criminal" if classified["case_type"] in ("刑事", "刑民交叉") else "civil"
        ...
        if lawyer_referral:
            return AnalysisResult([], complexity, True, lawyer_msg, "", trace_id, [], case_kind)

        # Step 3: RAG 检索（自动选库）
        retrieved = self.retriever.retrieve(case_text, k=5, kind=case_kind)
        ...
        retrieved_cases_info = [
            RetrievedCase(
                ...
                ruling_points=r["case"].get("metadata", {}).get("ruling_points", "")[:200],
                related_laws=r["case"].get("metadata", {}).get("related_laws", "")[:200],
        ...
        if not conclusions:
            return AnalysisResult(
                ...
                raw, trace_id, retrieved_cases_info, case_kind
            )

        return AnalysisResult(conclusions, complexity, False, "", raw, trace_id, retrieved_cases_info, case_kind)

    def to_dict(self, result: AnalysisResult, case_kind: str = "civil") -> dict:
        return {
            "trace_id": result.trace_id,
            "complexity": result.complexity,
            "lawyer_referral": result.lawyer_referral,
            "lawyer_message": result.lawyer_message,
            "retrieved_cases": [asdict(rc) for rc in result.retrieved_cases],
            "case_kind": case_kind,
            "conclusions": [
                {
                    "content": c.content,
                    "citations": [{"type": cit.type, "id": cit.id, "text": cit.text[:80]} for cit in c.citations]
