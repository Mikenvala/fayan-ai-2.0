    ...
    if _fayan is None:
        ...
        try:
            _fayan = FaYanLegal(
                ...
                criminal_json=CRIMINAL_CASES_JSON
            )
            _init_error = None
...
def status():
    ...
    fayan = get_fayan()
    import json, os
    cases_path = os.path.join(os.path.dirname(__file__), "..", "extracted_cases", "merged_all_cases.json")
    raw_count = 0
    try:
    ...
    return jsonify({
        ...
        "error": _init_error,
        "cases_count": fayan.retriever.civil_count if fayan else 0,
        "criminal_count": fayan.retriever.criminal_count if fayan else 0,
        "raw_cases_count": raw_count,
        "model": LLM_MODEL,
    })
...
def analyze():
    ...
    try:
        ...
        return jsonify(get_fayan().to_dict(result, result.case_kind))
    except Exception as e:
        import traceback
        traceback.print_exc()
