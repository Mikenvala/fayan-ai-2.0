...
"""
多模型法律能力对比评测
支持: MiniMax-M2.7 / DeepSeek-Chat / Kimi (Moonshot) / 智谱 GLM-4-Flash
"""
...
def run_model(cfg):
    ...
    if not api_key:
        print(f"  ⚠️  {cfg['name']}: 无API Key，跳过")
        return None
    ...
    for i, q in enumerate(QUESTIONS, 1):
        ...
        try:
            ...
            results.append({**q, "answer": answer[:500], "score": score})
    ...
    print(f"  >>> 平均分: {avg:.2f}/5 ({avg/5*100:.0f}%)")
    cat_str = " | ".join(f"{c}:{sum(v)/len(v):.1f}" for c,v in cats.items())
    print(f"  >>> 分类: {cat_str}")
...
if __name__ == "__main__":
    ...
    path = os.path.join(out, "multi_benchmark_results.json")
    with open(path, "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)

    print(f"\n{'='*55}")
    print(f"  ✅ 评测完成！结果已保存: {path}")
    print(f"  📊 模型数: {len(all_results)}")
    for name, data in all_results.items():
        print(f"     {name}: {data['avg_score']:.2f}/5 ({data['accuracy']:.0f}%)")
